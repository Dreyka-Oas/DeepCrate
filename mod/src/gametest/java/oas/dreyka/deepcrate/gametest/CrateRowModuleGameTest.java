...
public class CrateRowModuleGameTest {
    ...
    @GameTest
    public void theLidStaysQuietWhileTheScreenIsRebuilt(GameTestHelper gameTestHelper) {
        ServerPlayer serverPlayer = gameTestHelper.makeMockServerPlayerInLevel();
        DeepCrateBlockEntity deepCrateBlockEntity = placeCrate(gameTestHelper);
        Vec3 atTheCrate = gameTestHelper.absoluteVec(Vec3.atCenterOf(CRATE));
        serverPlayer.snapTo(atTheCrate.x, atTheCrate.y, atTheCrate.z);

        deepCrateBlockEntity.startOpen(serverPlayer);
        int afterARealOpen = deepCrateBlockEntity.lidSoundsPlayed();

        // What a row module change does: close the old menu, open the new one, in one call.
        deepCrateBlockEntity.lidSilently(() -> {
            deepCrateBlockEntity.stopOpen(serverPlayer);
            deepCrateBlockEntity.startOpen(serverPlayer);
        });

        assertEquals(gameTestHelper, 1, afterARealOpen, "sounds after a real open");
        assertEquals(gameTestHelper, 1, deepCrateBlockEntity.lidSoundsPlayed(), "sounds after the rebuild");

        deepCrateBlockEntity.stopOpen(serverPlayer);
        assertEquals(gameTestHelper, 2, deepCrateBlockEntity.lidSoundsPlayed(), "the real close still speaks");
        gameTestHelper.succeed();
    }
